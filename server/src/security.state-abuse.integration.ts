...
beforeAll(async () => {
  ...
  await db.insert(schema.organization).values([
    { id: ids.university, type: "university", name: "Abuse Test University", slug: "sec-abuse-university" },
...
async function cleanup(schema: Awaited<typeof import("./db/schema")>) {
  ...
  await db.delete(schema.placementEvaluation).where(inArray(schema.placementEvaluation.placementId, placementIds));
  await db.delete(schema.placementDocument).where(inArray(schema.placementDocument.placementId, placementIds));
  await db.delete(schema.progressReport).where(inArray(schema.progressReport.placementId, placementIds));
  await db.delete(schema.attendanceAdjustmentRequest).where(inArray(schema.attendanceAdjustmentRequest.attendanceId, db.select({ id: schema.attendanceRecord.id }).from(schema.attendanceRecord).where(inArray(schema.attendanceRecord.placementId, placementIds))));
  await db.delete(schema.attendanceRecord).where(inArray(schema.attendanceRecord.placementId, placementIds));
  await db.delete(schema.placementWorkSchedule).where(eq(schema.placementWorkSchedule.id, ids.schedule));
  await db.delete(schema.placement).where(inArray(schema.placement.id, placementIds));
  await db.delete(schema.internshipApplication).where(inArray(schema.internshipApplication.id, applicationIds));
  ...
  await db.delete(schema.internshipApplication).where(eq(schema.internshipApplication.internshipId, ids.capacityInternship));
  await db.delete(schema.internship).where(inArray(schema.internship.id, internshipIds));
  await db.delete(schema.onboardingRequest).where(inArray(schema.onboardingRequest.userId, userIds));
  await db.delete(schema.organizationMembership).where(inArray(schema.organizationMembership.organizationId, orgIds));
...
describe("terminal-state abuse: a completed placement cannot be reopened or re-transitioned", () => {
  it("cannot move a completed placement back to active", async () => {
    ...
    const { DrizzlePlacementRepository } = await import("./modules/placements/placement.repository");
    const service = new PlacementService(new DrizzlePlacementRepository(db));
    await expect(service.updateStatus(users.uniAdmin, ids.placement, "active")).rejects.toMatchObject({ code: "INVALID_PLACEMENT_TRANSITION" });
  });
  ...
  it("cannot cancel an already-completed placement", async () => {
    ...
    const { DrizzlePlacementRepository } = await import("./modules/placements/placement.repository");
    const service = new PlacementService(new DrizzlePlacementRepository(db));
    await expect(service.updateStatus(users.uniAdmin, ids.placement, "cancelled")).rejects.toMatchObject({ code: "INVALID_PLACEMENT_TRANSITION" });
  });
});
...
describe("duplicate-action abuse: attendance", () => {
  it("a second check-in for the same day is rejected by the DB unique constraint, not silently duplicated", async () => {
    ...
    const { DrizzleAttendanceRepository } = await import("./modules/attendance/attendance.repository");
    ...
    await expect(service.checkIn(users.student, ids.placementActive, {})).rejects.toMatchObject({ code: "ATTENDANCE_ALREADY_EXISTS" });
  });
  ...
  it("a second pending adjustment request for the same attendance record is rejected", async () => {
    ...
    const record = await db.query.attendanceRecord.findFirst({ where: eq(schema.attendanceRecord.placementId, ids.placementActive) });
    const service = new AttendanceService(new DrizzleAttendanceRepository(db));
    await service.requestAdjustment(users.student, record!.id, { reason: "First adjustment request, long enough reason." });
    await expect(
      service.requestAdjustment(users.student, record!.id, { reason: "Second adjustment request while first still pending." }),
...
describe("duplicate-action abuse: documents", () => {
  it("a document that was already reviewed cannot be reviewed again (flip approved->rejected after the fact)", async () => {
    ...
    const [document] = await db.insert(schema.placementDocument).values({
      placementId: ids.placementActive,
      studentUserId: users.student,
      type: "consent",
    ...
    await expect(service.review(users.supervisor, document!.id, "rejected", "Trying to flip the decision.")).rejects.toMatchObject({ code: "DOCUMENT_ALREADY_REVIEWED" });
  });
});
...
describe("duplicate-action abuse: evaluations", () => {
  it("a submitted evaluation cannot be submitted again, and becomes immutable to further save() calls", async () => {
    ...
    const { DrizzleEvaluationRepository } = await import("./modules/evaluations/evaluation.repository");
    const service = new EvaluationService(new DrizzleEvaluationRepository(db));
    const draft = await service.save(users.advisor, ids.placementActive, { technicalScore: 4, communicationScore: 4, responsibilityScore: 4, comment: "Solid performance." });
    const submitted = await service.submit(users.advisor, draft.id);
    expect(submitted.status).toBe("submitted");
    await expect(service.submit(users.advisor, draft.id)).rejects.toMatchObject({ code: "EVALUATION_IMMUTABLE" });
    await expect(
      service.save(users.advisor, ids.placementActive, { technicalScore: 1, communicationScore: 1, responsibilityScore: 1, comment: "Trying to downgrade after submission." }),
...
describe("duplicate/invalid-state abuse: progress reports", () => {
  it("a submitted report's content cannot be edited", async () => {
    ...
    const draft = await service.create({ actorUserId: users.student, placementId: ids.placementActive, periodStart: new Date("2026-09-01"), periodEnd: new Date("2026-09-07"), summary: "Week one activities, long enough to pass validation.", hoursWorked: 40 });
    await service.submit(users.student, draft!.id);
    await expect(
      service.update(users.student, draft!.id, { summary: "Trying to edit after submission, long enough text." }),
  ...
  it("a draft (not-yet-submitted) report cannot be reviewed", async () => {
    ...
    const { DrizzleProgressRepository } = await import("./modules/progress/progress.repository");
    const service = new ProgressService(new DrizzleProgressRepository(db));
    const draft = await service.create({ actorUserId: users.student, placementId: ids.placementActive, periodStart: new Date("2026-09-08"), periodEnd: new Date("2026-09-14"), summary: "Week two activities, long enough to pass validation.", hoursWorked: 40 });
    await expect(service.review(users.advisor, draft!.id, "approved")).rejects.toMatchObject({ code: "INVALID_PROGRESS_TRANSITION" });
  });
});
...
describe("duplicate-action abuse: internship application acceptance beyond capacity", () => {
  it("cannot accept a second application once capacity (1) is already filled, even racing the row-locking transaction", async () => {
    const schema = await import("./db/schema");
    const { DrizzleInternshipRepository } = await import("./modules/internships/internship.repository");
    await db.insert(schema.internship).values({ id: ids.capacityInternship, companyOrganizationId: ids.company, createdByUserId: users.supervisor, title: "Capacity abuse internship", description: "Fixture only.", location: "Bangkok", workMode: "onsite", capacity: 1, applicationDeadline: new Date("2027-01-01"), status: "published" });
    const [appOne] = await db.insert(schema.internshipApplication).values({ internshipId: ids.capacityInternship, studentUserId: users.student, universityOrganizationId: ids.university, statement: "First applicant statement, long enough text.", status: "under_review" }).returning();
    const [appTwo] = await db.insert(schema.internshipApplication).values({ internshipId: ids.capacityInternship, studentUserId: users.secondApplicant, universityOrganizationId: ids.university, statement: "Second applicant statement, long enough text.", status: "under_review" }).returning();
...
describe("duplicate-action abuse: onboarding", () => {
  it("a second onboarding request cannot be created while one is already pending", async () => {
    ...
    const { DrizzleOnboardingRepository } = await import("./modules/onboarding/onboarding.repository");
    const service = new OnboardingService(new DrizzleOnboardingRepository(db));
    await service.submit(users.onboardingDuplicate, { requestedRole: "advisor", targetOrganizationId: ids.university, profile: { fullName: "Duplicate Test", email: "dupe@example.test", phone: "0800000000", faculty: "Engineering", department: "CS", academicTitle: undefined, employeeId: undefined } });
    await expect(
      service.submit(users.onboardingDuplicate, { requestedRole: "advisor", targetOrganizationId: ids.university, profile: { fullName: "Duplicate Test", email: "dupe@example.test", phone: "0800000000", faculty: "Engineering", department: "CS", academicTitle: undefined, employeeId: undefined } }),
