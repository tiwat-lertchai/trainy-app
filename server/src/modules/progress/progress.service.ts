...
export class ProgressService {
  ...
  async update(
    actorUserId: string,
    reportId: string,
    changes: Partial<Pick<ProgressRecord, "periodStart" | "periodEnd" | "summary" | "hoursWorked">>,
  ) {
    ...
    if (
      ...
      throw new AppError("Submitted report content is immutable", 409, "PROGRESS_REPORT_IMMUTABLE");
    this.requirePeriod(
  ...
  async review(
    ...
    if (![placement.advisorUserId, placement.supervisorUserId].includes(actorUserId))
      throw new AppError("Assigned reviewer access is required", 403, "PROGRESS_REVIEWER_REQUIRED");
    ...
    if (decision === "revision_requested" && !feedback?.trim())
      throw new AppError("Feedback is required", 422, "PROGRESS_FEEDBACK_REQUIRED");
    const reviewed = await this.repository.update(report.id, {
      status: decision,
  ...
  private async requireReport(id: string) {
    ...
    if (!record)
      throw new AppError("Progress report was not found", 404, "PROGRESS_REPORT_NOT_FOUND");
    return record;
  }
