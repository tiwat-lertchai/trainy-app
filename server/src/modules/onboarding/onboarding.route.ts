...
export const onboardingRoute = new Hono<{ Variables: AuthVariables }>()
  ...
  .post("/", zValidator("json", submitOnboardingSchema), async (c) =>
    c.json({ data: await service.submit(c.get("authUser").id, c.req.valid("json")) }, 201),
  )
  .get("/reviews", async (c) => c.json({ data: await service.listReviews(c.get("authUser").id) }))
  .post(
    "/:onboardingId/review",
    zValidator("param", onboardingIdParamSchema),
    zValidator("json", reviewOnboardingSchema),
    async (c) =>
      c.json({
        data: await service.review(
          c.get("authUser").id,
          c.req.valid("param").onboardingId,
          c.req.valid("json"),
        ),
      }),
  );
