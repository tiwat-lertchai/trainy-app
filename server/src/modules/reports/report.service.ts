...
export class ReportService {
  ...
  async organizationSummary(actorUserId: string, organizationId: string) {
    ...
    const adminRole =
      access?.organizationType === "university"
        ? "university_admin"
        : "company_admin";
    ...
    const [activeMembers, applications, placements, internships] =
      await Promise.all([
        this.repository.countMembers(organizationId),
        this.repository.applicationCounts(
          organizationId,
          access.organizationType,
        ),
        this.repository.placementCounts(
          organizationId,
          access.organizationType,
        ),
