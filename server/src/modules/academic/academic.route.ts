...
export const academicRoute = new Hono<{ Variables: AuthVariables }>()
  .use("*", requireAuth)
  .get("/:organizationId/faculties", zValidator("param", organizationIdParamSchema), async (c) =>
    c.json({ data: await service.listFaculties(c.req.valid("param").organizationId) }),
  )
  .post(
  ...
  .get("/faculties/:facultyId/majors", zValidator("param", facultyIdParamSchema), async (c) =>
    c.json({ data: await service.listMajors(c.req.valid("param").facultyId) }),
  )
  .post(
