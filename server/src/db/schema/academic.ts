...
export const academicMajor = pgTable("academic_major", {
  id: uuid("id").defaultRandom().primaryKey(),
  facultyId: uuid("faculty_id").notNull().references(() => academicFaculty.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
export const academicFacultyRelations = relations(academicFaculty, ({ one, many }) => ({
  organization: one(organization, { fields: [academicFaculty.organizationId], references: [organization.id] }),
  majors: many(academicMajor),
}));

export const academicMajorRelations = relations(academicMajor, ({ one }) => ({
  faculty: one(academicFaculty, { fields: [academicMajor.facultyId], references: [academicFaculty.id] }),
}));
