...
async function cleanup(schema: Awaited<typeof import("./db/schema")>) {
  ...
  await db.delete(schema.placementEvaluation).where(eq(schema.placementEvaluation.placementId, ids.placementA));
  await db.delete(schema.placementDocument).where(eq(schema.placementDocument.placementId, ids.placementA));
  await db.delete(schema.progressReport).where(eq(schema.progressReport.placementId, ids.placementA));
  await db.delete(schema.attendanceAdjustmentRequest).where(inArray(schema.attendanceAdjustmentRequest.attendanceId, db.select({ id: schema.attendanceRecord.id }).from(schema.attendanceRecord).where(eq(schema.attendanceRecord.placementId, ids.placementA))));
  await db.delete(schema.attendanceRecord).where(eq(schema.attendanceRecord.placementId, ids.placementA));
  await db.delete(schema.placementWorkSchedule).where(eq(schema.placementWorkSchedule.placementId, ids.placementA));
  await db.delete(schema.placement).where(eq(schema.placement.id, ids.placementA));
  await db.delete(schema.internshipApplication).where(eq(schema.internshipApplication.id, ids.applicationA));
...
describe("cross-tenant IDOR: an outsider (student B, unrelated university) against placement A", () => {
  it("cannot list placement A's attendance", async () => {
    ...
    await expect(service.list(users.studentB, ids.placementA)).rejects.toMatchObject({ code: "PLACEMENT_ACCESS_REQUIRED" });
  });
  ...
  it("cannot check in against placement A (privacy-preserving 404, not a leaky 403)", async () => {
    ...
    const { DrizzleAttendanceRepository } = await import("./modules/attendance/attendance.repository");
    const service = new AttendanceService(new DrizzleAttendanceRepository(db));
    await expect(service.checkIn(users.studentB, ids.placementA, {})).rejects.toMatchObject({ code: "ATTENDANCE_NOT_FOUND" });
  });
  ...
  it("cannot create a progress report against placement A (privacy-preserving 404)", async () => {
    ...
    const service = new ProgressService(new DrizzleProgressRepository(db));
    await expect(
      service.create({ actorUserId: users.studentB, placementId: ids.placementA, periodStart: new Date("2026-10-01"), periodEnd: new Date("2026-10-07"), summary: "Attempted cross-tenant write, long enough text.", hoursWorked: 40 }),
  ...
  it("cannot download a document that belongs to placement A", async () => {
    ...
    const [document] = await db.insert((await import("./db/schema")).placementDocument).values({
    ...
    await expect(service.download(users.studentB, document!.id)).rejects.toMatchObject({ code: "PLACEMENT_ACCESS_REQUIRED" });
  });
  ...
  it("cannot list university A's placements without membership there", async () => {
    ...
    await expect(service.listOrganizationPlacements(users.studentB, ids.universityA)).rejects.toMatchObject({ code: "ORGANIZATION_ACCESS_REQUIRED" });
  });
  ...
  it("cannot read university A's organization report/summary", async () => {
    ...
    await expect(service.organizationSummary(users.studentB, ids.universityA)).rejects.toMatchObject({ code: "ORGANIZATION_ADMIN_REQUIRED" });
  });
  ...
  it("an admin of university B cannot read university A's report either (tenant boundary, not just role check)", async () => {
    ...
    const { DrizzleReportRepository } = await import("./modules/reports/report.repository");
    const service = new ReportService(new DrizzleReportRepository(db));
    await expect(service.organizationSummary(users.uniBAdmin, ids.universityA)).rejects.toMatchObject({ code: "ORGANIZATION_ADMIN_REQUIRED" });
  });
});
...
describe("role escalation: a legitimate participant of placement A acting outside their role", () => {
  it("the student cannot review their own submitted document (advisor/supervisor-only action)", async () => {
    ...
    const service = new DocumentService(repository);
    await expect(service.review(users.studentA, document!.id, "approved")).rejects.toMatchObject({ code: "DOCUMENT_REVIEWER_REQUIRED" });
  });
  ...
  it("the student cannot score their own evaluation (advisor/supervisor-only action)", async () => {
    ...
    const service = new EvaluationService(new DrizzleEvaluationRepository(db));
    await expect(
      service.save(users.studentA, ids.placementA, { technicalScore: 5, communicationScore: 5, responsibilityScore: 5, comment: "Self-scoring attempt." }),
  ...
  it("an advisor cannot set the company work schedule (company_admin-only action)", async () => {
    ...
    const service = new AttendanceService(new DrizzleAttendanceRepository(db));
    await expect(
      service.saveSchedule(users.advisorA, ids.placementA, { days: [{ weekday: 1, startMinute: 480, endMinute: 1020, breakMinutes: 60, graceMinutes: 10 }], timezone: "Asia/Bangkok", locationPolicy: "disabled" }),
  ...
  it("a coordinator cannot create academic faculties (university_admin-only, not any university staff)", async () => {
    ...
    await expect(service.createFaculty(users.coordinatorA, ids.universityA, "Escalation attempt faculty")).rejects.toMatchObject({ code: "ORGANIZATION_ADMIN_REQUIRED" });
  });
  ...
  it("a student cannot use the organization-wide placement listing endpoint, even for their own university", async () => {
    ...
    const { DrizzlePlacementRepository } = await import("./modules/placements/placement.repository");
    const service = new PlacementService(new DrizzlePlacementRepository(db));
    await expect(service.listOrganizationPlacements(users.studentA, ids.universityA)).rejects.toMatchObject({ code: "ORGANIZATION_ACCESS_REQUIRED" });
  });
});
