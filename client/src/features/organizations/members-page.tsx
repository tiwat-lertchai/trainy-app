...
const universityRoles: OrganizationRole[] = ["university_admin", "coordinator", "advisor", "student"];
...
export function MembersPage() {
	...
	const members = useQuery({
		...
		queryFn: async () => { const r = await apiClient.api.v1.organizations[":organizationId"].members.$get({ param: { organizationId: organizationId! } }); if (!r.ok) throw new Error(); return r.json(); },
		enabled: Boolean(organizationId) && isAdmin,
	});

	const addMember = useMutation({
		mutationFn: async (input: { userId: string; role: OrganizationRole }) => { const r = await apiClient.api.v1.organizations[":organizationId"].members.$post({ param: { organizationId: organizationId! }, json: input }); if (!r.ok) throw new Error(`MEMBER_${r.status}`); return r.json(); },
	...
	return (
		<div>
			...
			<p className="mt-2 text-muted-foreground">จัดการสิทธิ์และสถานะสมาชิกของ {context?.organization.name}</p>

			<form className="mt-6 grid gap-3 rounded-2xl border bg-white p-6 sm:grid-cols-[1fr_220px_auto] sm:items-end" onSubmit={submitAddMember}>
				<label className="grid gap-2 text-sm font-semibold">รหัสผู้ใช้ (User ID)
					<input name="userId" required placeholder="รหัสบัญชีผู้ใช้ที่มีอยู่แล้วในระบบ" className="h-11 rounded-xl border bg-background px-3 font-normal outline-none focus:border-primary focus:ring-3 focus:ring-primary/10" />
				</label>
				<label className="grid gap-2 text-sm font-semibold">บทบาท
			...
			{addMember.isError && <p role="alert" className="mt-2 text-sm text-destructive">เพิ่มสมาชิกไม่สำเร็จ ตรวจสอบรหัสผู้ใช้และสิทธิ์อีกครั้ง</p>}

			{members.isLoading && <div className="mt-8 h-32 animate-pulse rounded-2xl bg-muted" />}
			{members.data?.data.length === 0 && <div className="mt-8 rounded-2xl border bg-white p-10 text-center text-muted-foreground">ยังไม่มีสมาชิก</div>}

			<div className="mt-6 grid gap-3">
				{members.data?.data.map((member) => {
					const view = member as typeof member & { user?: { id: string; name: string; email: string } };
					return (
						<article key={member.id} className="flex flex-col gap-3 rounded-2xl border bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
							<div className="flex items-center gap-3">
								<span className="grid size-11 place-items-center rounded-xl bg-[#edf3ff] text-primary"><UsersRound /></span>
								<div>
									<p className="font-bold">{view.user?.name ?? member.userId}</p>
									<p className="text-sm text-muted-foreground">{view.user?.email ?? member.userId}</p>
								</div>
							</div>
							<div className="flex flex-wrap items-center gap-2">
								<select className="h-10 rounded-lg border bg-white px-3 text-sm" value={member.role} disabled={updateMember.isPending} onChange={(event) => updateMember.mutate({ membershipId: member.id, role: event.target.value as OrganizationRole })}>
									{availableRoles.map((role) => <option key={role} value={role}>{roleLabels[role]}</option>)}
								</select>
