...
export function ApplicationPage() {
	...
	const [confirmation, setConfirmation] = useState<{
		id: string;
		action: "withdrawn" | "accepted" | "rejected";
	} | null>(null);
	...
	const universityApplications = useQuery({
		...
		enabled: Boolean(
			organizationId && ["university_admin", "coordinator", "advisor"].includes(role ?? ""),
		),
	});
	...
	const companyApplications = useQuery({
		...
		queryFn: async () => {
			const internshipsResponse = await apiClient.api.v1.internships.companies[
				":organizationId"
			].$get({ param: { organizationId: organizationId! } });
			...
			const groups = await Promise.all(
				internships.map(async (internship) => {
					const response = await apiClient.api.v1.internships[":internshipId"].applications.$get({
	...
	const mutation = useMutation({
		mutationFn: async ({
			...
			id: string;
			action: "withdrawn" | "under_review" | "accepted" | "rejected";
		}) => {
			const response =
				action === "withdrawn"
					? await apiClient.api.v1.internships.applications[":applicationId"].withdraw.$post({
							param: { applicationId: id },
						})
					: await apiClient.api.v1.internships.applications[":applicationId"].status.$patch({
							param: { applicationId: id },
							json: { status: action },
						});
		...
		<div>
			...
			<div className="mt-8 grid gap-4">
				{applications.data?.data.map((application) => {
					...
					return (
						<article key={application.id} className="rounded-2xl border bg-white p-5 sm:p-6">
							...
							<div className="mt-5 flex flex-wrap gap-2">
								{role === "student" && canWithdrawApplication(status) && (
									<Button
										variant="outline"
								...
								{actions.map((action) => (
									<Button
										...
										onClick={() =>
											action === "under_review"
												? mutation.mutate({ id: application.id, action })
			...
			<ConfirmationDialog
				...
				destructive={confirmation?.action !== "accepted"}
...
function Notice({ message, destructive = false }: { message: string; destructive?: boolean }) {
	return (
		<div
			...
			className={`mt-8 rounded-2xl border bg-white p-10 text-center ${destructive ? "border-destructive/20 text-destructive" : "text-muted-foreground"}`}
		>
			{message}
		</div>
	);
