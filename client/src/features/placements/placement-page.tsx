...
export function PlacementPage() {
	...
	const placements = useQuery({
		...
		queryFn: async () => { const response = isStudent ? await apiClient.api.v1.placements.me.$get() : await apiClient.api.v1.placements.organizations[":organizationId"].$get({ param: { organizationId: organizationId! } }); if (!response.ok) throw new Error("PLACEMENTS_FAILED"); return response.json(); },
		enabled: Boolean(role && (isStudent || organizationId)),
	});
	const members = useQuery({
		queryKey: ["organizations", organizationId, "members"],
		queryFn: async () => { const response = await apiClient.api.v1.organizations[":organizationId"].members.$get({ param: { organizationId: organizationId! } }); if (!response.ok) throw new Error("MEMBERS_FAILED"); return response.json(); },
		enabled: Boolean(organizationId && !isStudent),
	});
	const accepted = useQuery({
		queryKey: ["applications", "accepted", organizationId],
		queryFn: async () => { const response = await apiClient.api.v1.internships.universities[":organizationId"].applications.$get({ param: { organizationId: organizationId! } }); if (!response.ok) throw new Error("APPLICATIONS_FAILED"); return response.json(); },
		enabled: Boolean(organizationId && universityManagers.includes(role ?? "")),
	});
	const mutate = useMutation({
		mutationFn: async (input: { kind: "create"; applicationId: string; startDate: string; endDate: string } | { kind: "advisor"; placementId: string; userId: string } | { kind: "supervisor"; placementId: string; userId: string } | { kind: "status"; placementId: string; status: "active" | "completed" | "cancelled" }) => {
			if (input.kind === "create") { const response = await apiClient.api.v1.placements.$post({ json: { applicationId: input.applicationId, startDate: new Date(input.startDate).toISOString(), endDate: new Date(input.endDate).toISOString() } }); if (!response.ok) throw new Error(`PLACEMENT_${response.status}`); return response.json(); }
			if (input.kind === "advisor") { const response = await apiClient.api.v1.placements[":placementId"].advisor.$patch({ param: { placementId: input.placementId }, json: { advisorUserId: input.userId } }); if (!response.ok) throw new Error(`PLACEMENT_${response.status}`); return response.json(); }
	...
	return <div><div><p className="text-sm font-semibold text-primary">PLACEMENTS</p><h1 className="mt-2 text-3xl font-black">การฝึกงาน</h1><p className="mt-2 text-muted-foreground">มอบหมายผู้ดูแล กำหนดช่วงเวลา และติดตามสถานะการฝึกงาน</p></div>
		{universityManagers.includes(role ?? "") && acceptedItems.length > 0 && <section className="mt-8 rounded-2xl border bg-white p-6"><h2 className="font-bold">สร้างการฝึกงานจากใบสมัครที่ผ่าน</h2><div className="mt-4 grid gap-3">{acceptedItems.map((application) => <form key={application.id} className="grid gap-3 rounded-xl bg-muted p-4 sm:grid-cols-[1fr_160px_160px_auto] sm:items-end" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutate.mutate({ kind: "create", applicationId: application.id, startDate: String(data.get("startDate")), endDate: String(data.get("endDate")) }); }}><div><p className="font-semibold">{(application as typeof application & { internship?: { title: string }; student?: { name: string } }).internship?.title ?? application.id.slice(0, 8)}</p><p className="text-sm text-muted-foreground">{(application as typeof application & { student?: { name: string } }).student?.name ?? "นักศึกษา"}</p></div><DateField name="startDate" label="เริ่มฝึกงาน" /><DateField name="endDate" label="สิ้นสุด" /><Button disabled={mutate.isPending}>สร้าง Placement</Button></form>)}</div></section>}
...
function DateField({ name, label }: { name: string; label: string }) { return <label className="grid gap-1 text-xs font-semibold">{label}<input type="date" name={name} required className="h-10 rounded-lg border bg-white px-3 text-sm font-normal" /></label>; }
function Info({ icon: Icon, label, value }: { icon: typeof CalendarDays; label: string; value: string }) { return <div className="rounded-xl bg-muted p-4"><span className="flex items-center gap-2 text-xs text-muted-foreground"><Icon className="size-4" />{label}</span><p className="mt-2 font-semibold">{value}</p></div>; }
function Badge({ status }: { status: PlacementStatus }) { const labels = { pending: "รอมอบหมาย", active: "กำลังฝึกงาน", completed: "เสร็จสิ้น", cancelled: "ยกเลิก" }; return <span className={`h-fit w-fit rounded-full px-3 py-1 text-xs font-semibold ${status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{labels[status]}</span>; }
