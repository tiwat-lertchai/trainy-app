...
import { facultiesEnabledForRole, organizationFieldLabel, organizationTypeForRole, roleOptions, type OnboardingRole } from "./onboarding-rules";
...
async function loadFaculties(organizationId: string) {
	const response = await apiClient.api.v1.academic[":organizationId"].faculties.$get({ param: { organizationId } });
	if (!response.ok) throw new Error("FACULTIES_FAILED");
	return response.json();
}

async function loadMajors(facultyId: string) {
	const response = await apiClient.api.v1.academic.faculties[":facultyId"].majors.$get({ param: { facultyId } });
...
export function OnboardingPage() {
	...
	const effectiveRole = role ?? (mine.data?.data?.status === "revision_requested" ? mine.data.data.requestedRole as OnboardingRole : undefined);
	const facultiesEnabled = Boolean(organizationId) && Boolean(effectiveRole) && facultiesEnabledForRole(effectiveRole!);
	const faculties = useQuery({ queryKey: ["academic", "faculties", organizationId], queryFn: () => loadFaculties(organizationId), enabled: facultiesEnabled });
	const majors = useQuery({ queryKey: ["academic", "majors", facultyId], queryFn: () => loadMajors(facultyId), enabled: facultiesEnabled && effectiveRole === "student" && Boolean(facultyId) });
	...
	function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		if (selectedRole === "student") payload = { requestedRole: selectedRole, targetOrganizationId: value("organizationId"), profile: { ...profile, studentId: value("studentId"), faculty: facultyName, major: majorName, yearLevel: value("yearLevel") } };
		else if (selectedRole === "advisor") payload = { requestedRole: selectedRole, targetOrganizationId: value("organizationId"), profile: { ...profile, faculty: facultyName, department: value("department"), academicTitle: value("academicTitle") || undefined, employeeId: value("employeeId") || undefined } };
...
function RoleFields({ role, organizationId, facultyId, onFacultyChange, faculties, majors }: { role: OnboardingRole; organizationId: string; facultyId: string; onFacultyChange: (id: string) => void; faculties: FacultyOption[]; majors: FacultyOption[] }) {
	const facultySelect = <label className="grid gap-2 text-sm font-semibold">คณะ<select name="facultyId" required value={facultyId} onChange={(event) => onFacultyChange(event.target.value)} disabled={!organizationId} className="h-11 rounded-xl border bg-background px-3 font-normal disabled:opacity-50"><option value="">{organizationId ? "เลือกคณะ" : "เลือกองค์กร/มหาวิทยาลัยก่อน"}</option>{faculties.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>;
	if (role === "student") return <><Field name="studentId" label="รหัสนักศึกษา" /><label className="grid gap-2 text-sm font-semibold">ชั้นปี<select name="yearLevel" required defaultValue="" className="h-11 rounded-xl border bg-background px-3 font-normal"><option value="" disabled>เลือกชั้นปี</option>{["1", "2", "3", "4", "5", "6"].map((year) => <option key={year} value={year}>ปี {year}</option>)}</select></label>{facultySelect}<label className="grid gap-2 text-sm font-semibold">สาขา<select name="majorId" required disabled={!facultyId} className="h-11 rounded-xl border bg-background px-3 font-normal disabled:opacity-50"><option value="">{facultyId ? "เลือกสาขา" : "เลือกคณะก่อน"}</option>{majors.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></>;
	if (role === "advisor") return <>{facultySelect}<Field name="department" label="ภาควิชา/สาขา" /><Field name="academicTitle" label="ตำแหน่งทางวิชาการ (ถ้ามี)" required={false} /><Field name="employeeId" label="รหัสบุคลากร (ถ้ามี)" required={false} /></>;
